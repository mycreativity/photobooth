...
from api.models.db import Booth, Event, hash_api_key
...
async def booth_websocket(websocket: WebSocket, booth_id: str):
    ...
    Auth: API key is validated in the register message (AC2), not
    in the URL query string.  The connection is provisional until
    a valid register message is received.
    ...
    # AC2: Auth happens in the register message, not at connect time.
    # Connection is provisional until authenticated.
    authenticated = False
        ...
        while True:
            ...
            if msg_type == "heartbeat":
                if not authenticated:
                    continue  # Ignore heartbeats before auth
                ...
                # AC2: Validate API key from the register message body
                async for db in get_db():
                    result = await db.execute(
                        select(Booth).where(Booth.booth_id == booth_id)
                    )
                ...
                if booth and booth.api_key_hash:
                    api_key = msg.get("api_key", "")
                    if not api_key or hash_api_key(api_key) != booth.api_key_hash:
                        logger.warning(
                            "WS auth rejected: invalid API key for booth=%s", booth_id
                        )
                        ...
                        return
                elif booth and not booth.api_key_hash:
                    logger.warning(
                        "Booth %s has no API key set — allowing (legacy mode)",
                        booth_id,
                    )
                ...
                # AC3: Auto-push event card config on reconnect
                if event_id:
                    async for db in get_db():
                        ev_result = await db.execute(
                            select(Event).where(Event.id == event_id)
                        ...
                        if event:
                            from api.api.booths import build_push_event_data
                            push_data = build_push_event_data(event)
                            await websocket.send_json(push_data)
                            logger.info(
                                "Auto-pushed event %s to booth %s on reconnect",
                                event.uid, booth_id,
                            )
